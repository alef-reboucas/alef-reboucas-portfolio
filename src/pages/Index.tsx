...
// Create SectionTitle component inline since the import was failing
const SectionTitle = ({ children }) => (
  <h2 className="text-3xl font-bold mb-6 text-[#F9C784]">{children}</h2>
);

// Define cases array inline since the import was failing
const cases = [
  {
    title: "Integração PIX",
    description: "Implementação de pagamentos via PIX em produtos financeiros.",
    link: "/case/pix"
  },
  {
    title: "Migração de Sistema",
    description: "Processo completo de migração de sistema legado para nova plataforma.",
    link: "/case/migracao"
  }
];
      ...
      <section id="cases" className="py-16 px-6 bg-[#2D3A61]">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cases.map((item) => (
              <Link to={item.link} key={item.title} className="card border border-[#3D4B81] rounded-lg p-6 hover:bg-[#3D4B81]/30 transition-colors fade-in">
